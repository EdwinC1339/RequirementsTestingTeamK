...
async function getPlaces(lat, lon, radius, delay) {
  ...
  try {
    let result = (await fetchWithTimeoutAndDelay(apiURL, {delay, timeout: 30000, method: "GET"})).json()
    return result;
  } catch (error) {
    console.error(error);
    return null;
...
async function getTown(coordinates) {
  ...
  const response = await fetchWithTimeoutAndDelay(url, {timeout: 30000, delay: 0});
